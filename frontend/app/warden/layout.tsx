...
export default function WardenLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#090D14]">
        <LoadingState message="Verifying warden permissions..." />
      </div>
    );
  }
  ...
  if (user.role !== "WARDEN" && user.role !== "SUPER_ADMIN" && user.role !== "STAFF") {
    return (
      <div className="min-h-screen flex bg-[#090D14] text-slate-100">
        ...
        <div className="flex-1 flex flex-col p-10 max-w-xl mx-auto my-auto">
          <ErrorState
            ...
            message="This section is reserved for Wardens and Operations Officers."
